...
execution_name="optuna-onlymiss-delta-stateful-tolerance-20-with-lookahead-global-tid-vulkansim"
...
do
	echo "Executing sbatch optuna_no_softmax_train_test_one_job.sh with arguments:" --output=${logs_dir_name}"/"${scene}"-%j.out" --error=${logs_dir_name}"/"${scene}"-error-%j.out" "--mem="$memoryPerJob "-c" 4 "-q" $queue $train_trace_directory"/"${scene}"-frame0.txt" $test_trace_directory"/"${scene}"-frame1.txt" ${models_dir_name}"/"${scene} ${graphs_dir_name}"/"${scene} ${experiments_dir_name}"/"${scene}
	sbatch --output=${logs_dir_name}"/"${scene}"-%j.out" --error=${logs_dir_name}"/"${scene}"-error-%j.out" --mem=$memoryPerJob -c 1 -q $queue optuna_no_softmax_train_test_one_job.sh $train_trace_directory"/"${scene}"-frame0.txt" $test_trace_directory"/"${scene}"-frame1.txt" ${models_dir_name}"/"${scene} ${graphs_dir_name}"/"${scene} ${experiments_dir_name}"/"${scene}
done